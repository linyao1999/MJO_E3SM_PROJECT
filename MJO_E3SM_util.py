...
def spacetime_power(data, segsize=96, noverlap=60, spd=1, lat_lim=15, remove_low=True):
    """
...
def wk_plot_sym(sym, tlt='', logflg=True, savflg=False, pltDispCurve=True, fb=[0,0.48], center0=False, filename='wk.png'):
    ...
    if logflg:
        if center0:
            ...
            c = ax.contourf(wavenumber, frequency, np.log10(sym.T), cmap='RdBu_r', levels=np.linspace(-3.2,2.4,15), vmin=-3.2, vmax=2.4)
    else:
        c = ax.contourf(wavenumber, frequency, sym.T, cmap='RdBu_r', levels=15)
